...
def requete_b24():
    ...
    i= 1
    j=1
    burkina24_url="https://fasozine.com/actualite/"
    bf="https://fasozine.com/author"
    response=requests.get(burkina24_url, headers=HEADERS)
    response.encoding=response.apparent_encoding

    if response.status_code==200:
        html=response.text
        fichier=open("fasozine.html","w",encoding='utf-8')
        fichier.write(html)
        fichier.close()
        
        donnee=BeautifulSoup(html,"html5lib")
        container=donnee.find("div", class_="jeg_main_content jeg_column col-sm-8")
        ...
        if container != None:
            ...
            for list_url in list_urls:
                if list_url not in urls and "https://fasozine.com/author" not in list_url:
                    ...
                    if url!="#":
                        if i< 3:
                            ...
                            if response1.status_code==200:
                                print("Fichier ",i)
                                nomdufichier="fasozine_fichier"+str(i)+".txt"
                                htlm_url=response1.text
                                donne_url=BeautifulSoup(htlm_url,"html5lib")
                                div_content=donne_url.find("div", class_="content-inner")
                                div_date_content=donne_url.find("div", class_="jeg_meta_date")
                                div_date=div_date_content.find("a")
                                div_auteur_content=donne_url.find("div", class_="jeg_meta_author")
                                div_auteur=div_auteur_content.find("a")
                                titre_container=donne_url.find("div", class_="entry-header")
                                titre_article=titre_container.find("h1")
                                
                                if titre_article!=None:
                                    list_titres=get_text(titre_article)
                                    ...
                                    for titre in titres:
                                        if titre!="":
                                            nomdufichier_titre="fasozine_fichier_titre"+str(i)+".txt"
                                            with open(nomdufichier_titre, "w", encoding="UTF-8") as fichier:
                                                fichier.write(titre) 
                                ...
                                for auteur_article in auteurs_articles:
                                    nomdufichier_auteur="fasozine_fichier_auteur"+str(i)+".txt"
                                ...
                                for date_de_sortie in dates_de_sortie:
                                    nomdufichier_date="fasozine_fichier_date"+str(i)+".txt"
    ...
    def lire_et_inserer():
        for i in range(1, 3):  # Boucler sur les fichiers de 1 à 2
            ...
            with open(f'fasozine_fichier_titre{i}.txt', 'r', encoding='utf-8') as file:
                titre = file.read().strip()

            # Lire l'auteur depuis le fichier
            with open(f'fasozine_fichier_auteur{i}.txt', 'r', encoding='utf-8') as file:
                auteur = file.read().strip()

            # Lire la date depuis le fichier
            with open(f'fasozine_fichier_date{i}.txt', 'r', encoding='utf-8') as file:
                date_sortie = file.read().strip()

            # Lire la description depuis le fichier
            with open(f'fasozine_fichier{i}.txt', 'r', encoding='utf-8') as file:
                description = file.read().strip()

            # Récupérer l'ID correspondant à l'URL
            burkina24_url="https://fasozine.com/"
