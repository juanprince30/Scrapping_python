...
def requete_b24():
    ...
    i= 1
    j=1
    burkina24_url="https://lefaso.net/"
    response=requests.get(burkina24_url, headers=HEADERS)
    response.encoding=response.apparent_encoding

    if response.status_code==200:
        html=response.text
        fichier=open("lefaso.html","w",encoding='utf-8')
        fichier.write(html)
        fichier.close()
        
        donnee=BeautifulSoup(html,"html5lib")
        container=donnee.find("div", class_="col-xs-12 col-sm-9 col-md-9 col-lg-9")
        ...
        if container != None:
            ...
            for l in list_titres:
                if l not in titres and l !="" and "document.write(getLFRub" not in l:
                    titres.append(l)
                    
            for titre in titres:
                if titre!="":
                    if j<3:
                        nomdufichier="lefaso_fichier_titre"+str(j)+".txt"
            ...
            for list_url in list_urls:
                if "https://lefaso.net/"+list_url not in urls and "spip.php?rubrique" not in list_url :
                    urls.append("https://lefaso.net/"+list_url)
                    ...
                    if url!="#":
                        if i< 3:
                            ...
                            if response1.status_code==200:
                                print("Fichier ",i)
                                nomdufichier="lefaso_fichier"+str(i)+".txt"
                                htlm_url=response1.text
                                donne_url=BeautifulSoup(htlm_url,"html5lib")
                                div_content=donne_url.find("div", class_="article_content")
                                div_date_content=donne_url.find("div", style="width:100%")
                                div_date=div_date_content.find("p")
                                nomdufichier_auteur="lefaso_fichier_auteur"+str(i)+".txt"
                                ...
                                for date_de_sortie in dates_de_sortie:
                                    nomdufichier_date="lefaso_fichier_date"+str(i)+".txt"
                                    message = "Date_de_sorie:"
                                    date_de_sortie_avec_message = message + "\n" + date_de_sortie
                                    with open(nomdufichier_date, "w", encoding="UTF-8") as fichier:
                                        fichier.write(date_de_sortie_avec_message)
    ...
    def lire_et_inserer():
        for i in range(1, 3):  # Boucler sur les fichiers de 1 à 2
            ...
            with open(f'lefaso_fichier_titre{i}.txt', 'r', encoding='utf-8') as file:
                titre = file.read().strip()

            # Lire l'auteur depuis le fichier
            with open(f'lefaso_fichier_auteur{i}.txt', 'r', encoding='utf-8') as file:
                auteur = file.read().strip()

            # Lire la date depuis le fichier
            with open(f'lefaso_fichier_date{i}.txt', 'r', encoding='utf-8') as file:
                date_sortie = file.read().strip()

            # Lire la description depuis le fichier
            with open(f'lefaso_fichier{i}.txt', 'r', encoding='utf-8') as file:
