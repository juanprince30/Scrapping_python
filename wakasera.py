...
def requete_b24():
    ...
    i= 1
    j=1
    burkina24_url="https://www.wakatsera.com/"
    response=requests.get(burkina24_url, headers=HEADERS)
    response.encoding=response.apparent_encoding

    if response.status_code==200:
        html=response.text
        fichier=open("wakatsera.html","w",encoding='utf-8')
        fichier.write(html)
        fichier.close()
        ...
        if container != None:
            ...
            for titre in titres:
                if titre!="":
                    if j<3:
                        nomdufichier="wakatsera_fichier_titre"+str(j)+".txt"
                    ...
                    if url!="#":
                        if i< 3:
                            ...
                            if response1.status_code==200:
                                print("Fichier ",i)
                                nomdufichier="wakatsera_fichier"+str(i)+".txt"
                                htlm_url=response1.text
                                donne_url=BeautifulSoup(htlm_url,"html5lib")
                                div_content=donne_url.find("div", class_="td-post-content td-pb-padding-side")
                                ...
                                for auteur_article in auteurs_articles:
                                    nomdufichier_auteur="wakatsera_fichier_auteur"+str(i)+".txt"
                                ...
                                for date_de_sortie in dates_de_sortie:
                                    nomdufichier_date="wakatsera_fichier_date"+str(i)+".txt"
    ...
    def lire_et_inserer():
        for i in range(1, 3):  # Boucler sur les fichiers de 1 à 2
            ...
            with open(f'wakatsera_fichier_titre{i}.txt', 'r', encoding='utf-8') as file:
                titre = file.read().strip()

            # Lire l'auteur depuis le fichier
            with open(f'wakatsera_fichier_auteur{i}.txt', 'r', encoding='utf-8') as file:
                auteur = file.read().strip()

            # Lire la date depuis le fichier
            with open(f'wakatsera_fichier_date{i}.txt', 'r', encoding='utf-8') as file:
                date_sortie = file.read().strip()

            # Lire la description depuis le fichier
            with open(f'wakatsera_fichier{i}.txt', 'r', encoding='utf-8') as file:
